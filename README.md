...
![Orchestration](https://img.shields.io/badge/LangChain_%2B_LiteLLM-indigo)
...
All three providers are routed through a single `Provider` enum and a `BaseExtractionPipeline` ABC, so the scoring engine cannot tell which provider produced an extraction — guaranteeing schema parity across cloud and local deployments.
...
* A **Batch Verification** tab presents all extracted fields in a single review table with confidence bars, source quotes, and inline PDF thumbnails. A review gate blocks scoring until every low-confidence field (`< 0.80`) is explicitly confirmed or corrected, and the full audit log is exportable as JSON for regulatory submissions.
* PII (SSN, phone, email, MRN) is regex-scrubbed before any text reaches an LLM.

### Technical Stack

* **Frontend**: Gradio (dark-theme, split-screen layout, custom CSS for clinical-grade fidelity)
* **LLM Backends**: OpenAI API, Claude API, Ollama (local)
* **Orchestration**: LangChain + LiteLLM for model-agnostic routing
* **PDF Parsing**: PyMuPDF (text + page rendering) + pdfplumber (table extraction)
