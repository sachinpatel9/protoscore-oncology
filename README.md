...
* A **Batch Verification** tab presents all extracted fields in a single review table with confidence bars, source quotes, and inline PDF thumbnails. Per-row HITL approval is the gate: every extraction row lands as `Pending`. Click **Edit Report** to enter edit mode, optionally correct any of the four numeric score-driver fields, then tick the **Action checkbox** on each row to flip it to `Confirmed`. `Confirm & Score` is blocked until no rows remain pending. The full audit log is exportable as JSON for regulatory submissions.
